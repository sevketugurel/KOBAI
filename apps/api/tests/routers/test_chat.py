...
@pytest.mark.asyncio
async def test_chat_stream_error_still_closes_with_done(monkeypatch):
    """Gemini vb. hata — chunked gövde yarım kalmadan [DONE] gelmeli."""
    monkeypatch.setenv("GEMINI_API_KEY", "k")
    from services.job_queue import queue
    from schemas.analysis import AnalysisResult
    ...
    async def boom(*a, **k):
        ...
        yield ""  # noqa: B901 — async generator; ilk adımda hata

    from routers import chat as cr

    monkeypatch.setattr(cr, "_stream_answer", boom)
    from main import app

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://t") as c:
        r = await c.post("/chat", json={"message": "selam", "job_id": job_id, "history": []})
    assert r.status_code == 200
    assert "data: [HATA]" in r.text
